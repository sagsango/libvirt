...
# Script to set up and run a VM using custom libvirt and QEMU in session mode (sudo-free)
...
# Requires p7zip-full for ISO extraction (install once: sudo apt install p7zip-full)
...
#if echo "$QEMU_VERSION" | grep -q "Debian"; then
#    echo "Error: System QEMU detected ($QEMU_VERSION). Expected custom QEMU at $QEMU_BINARY"
#    echo "Check PATH or rebuild QEMU in ~/local/qemu"
#    exit 1
#fi
...
# Step 7: Configure session-specific network (slirp)
